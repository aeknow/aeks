...
func ViewHome_old(ctx iris.Context, pubkey string, dbpath string) {
	//View the Homepage of a user
	accountname := SESS_GetAccountName(ctx)
	if !FileExist(dbpath) {
		ctx.HTML("<h1>No such sqlite db." + dbpath + "</h1>")
	...
	querystr := "SELECT aid,title,hash,abstract FROM aek ORDER BY lastmodtime DESC LIMIT 100"
	rows, err := db.Query(querystr)
	checkError(err)
	...
	for rows.Next() {
		err = rows.Scan(&aid, &title, &hash)
		checkError(err)

		//PageContent = PageContent + "<br />" + "<li><a href=/view?pubkey=" + pubkey + "&hash=" + hash + ">" + title + "</a></li>"
		if pubkey == accountname {
			PageContent = PageContent + "<li><a href=/view?pubkey=" + pubkey + "&hash=" + hash + "&aid=" + aid + ">" + title + "</a> - <a href=/editpage?pubkey=" + accountname + "&aid=" + aid + " target=_blank>EDIT</a> - <a href=/delpage?aid=" + aid + ">Del</a></li>"
		} else {
			if accountname == "" {
				PageContent = PageContent + "<li><a href=/view?pubkey=" + pubkey + "&hash=" + hash + "&aid=" + aid + ">" + title + "</a></li>"

			} else {
				PageContent = PageContent + "<li><a href=/view?pubkey=" + pubkey + "&hash=" + hash + "&aid=" + aid + ">" + title + "</a> - <a href=/editpage?action=fork&pubkey=" + pubkey + "&aid=" + aid + " target=_blank>Fork</a></li>"
	...
	myPage := PageList{Account: accountname, PageTitle: DB_GetPublicItem(pubkey, "name"), PageContent: template.HTML(PageContent)}

	ctx.ViewData("", myPage)
	myTheme := DB_GetConfigItem(accountname, "Theme")

	//ctx.View(myTheme + "/haeme_index.php")
	ctx.View(myTheme + "mainroad/haeme_index.php")
	db.Close()
...
func ViewTag_old(ctx iris.Context, tag string, pubkey string, dbpath string) {
	//View the tag list
	...
	querystr := "SELECT aid,title,author,keywords,aid,abstract,filetype,pubtime,hash FROM aek WHERE keywords LIKE '%" + tag + "%' OR title LIKE '%" + tag + "%' ORDER BY pubtime DESC"
	fmt.Println(querystr)
	rows, err := db.Query(querystr)
	checkError(err)
	var aid string
	...
	for rows.Next() {
		err = rows.Scan(&aid, &title, &author, &keywords, &aid, &abstract, &filetype, &pubtime, &hash)
		checkError(err)
		PageContent = PageContent + "<li><a href=/view?pubkey=" + pubkey + "&hash=" + hash + "&aid=" + aid + ">" + title + "</a> - <div class=pubtime>" + pubtime + "</div> <a href=/editpage?pubkey=" + author + "&aid=" + aid + " target=_blank>EDIT</a></li>"
	...
	myPage := PageList{Account: accountname, PageTitle: title, PageContent: template.HTML(PageContent)}

	ctx.ViewData("", myPage)
	myTheme := DB_GetConfigItem(accountname, "Theme")
	ctx.View(myTheme + "/haeme_pagelist.php")
	//ctx.View("haeme_pagelist.php")
	fmt.Println("tag")
}

func ViewCat_old(ctx iris.Context, tag string, pubkey string, dbpath string) {
	//View the tag list
	...
	querystr := "SELECT aid,title,author,keywords,aid,abstract,filetype,pubtime,hash FROM aek WHERE tags LIKE '%" + tag + "%' OR title LIKE '%" + tag + "%' ORDER BY pubtime DESC LIMIT 10"
	fmt.Println(querystr)
	rows, err := db.Query(querystr)
	checkError(err)
	var aid string
