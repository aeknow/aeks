    ...
    <div class="container container--outer">
        ...
        <div class="wrapper flex">
            <div class="primary">
			 ...
			 {{.Title}}
			</a> {{if gt (len .IsOwner) 1}}
                                <small><a href="editpage?pubkey={{.Pubkey}}&aid={{.Aid}}" target=_blank>Edit</a></small> {{else}}
                                <small><a href="editpage?pubkey={{.Pubkey}}&aid={{.Aid}}&action=fork" target=_blank>Fork</a></small> {{end}}
                ...
                <div class="pagination">
                    ...
                    <a class="pagination__item pagination__item--prev btn" href="{{ .Paginator.PrevURL }}">«</a> {{- end }}
                    <span class="pagination__item pagination__item--current">
		{{- .Paginator.PageNumber }}/{{ .Paginator.TotalPages -}}
	</span> {{- if .Paginator.HasNext }}
                    <a class="pagination__item pagination__item--next btn" href="{{ .Paginator.NextURL }}">»</a> {{- end }}
                </div>
                {{- end }}
            ...
            <aside class="sidebar">
                <div class="widget-search widget">
                    <form class="widget-search__form" role="search" method="get" action="/search">
                        <label>
			<input class="widget-search__field" type="search" placeholder="SEARCH…" value="" name="keyword" aria-label="SEARCH…">
		</label>
                ...
                <div class="widget-recent widget">
                    ...
                    <div class="widget__content">
                        <ul class="widget__list">
                            ...
                            {{range .Posts}} {{if gt (len .Title) 0}}
                            <li class="widget__item"><a class="widget__link" href="/view?pubkey={{.Pubkey}}&hash={{.Hash}}&aid={{.Aid}}">{{.Title}}</a></li>
                            {{end}} {{end}}
                        </ul>
                    </div>
                </div>
